...
# MongoDB client accessed via global sync_client from database config
    ...
    def _handle_review_mode(self, job_id: str, dubbed_segments: list, transcript_id: str, 
        ...
        if manifest_override:
            ...
            manifest["add_subtitle_to_video"] = voice_config["add_subtitle_to_video"]
        else:
            manifest = build_manifest(
                ...
                add_subtitle_to_video=voice_config["add_subtitle_to_video"]
            )
    ...
    def _create_speaker_voice_clones(self, transcription_segments: List[Dict], unique_speakers: List[str], 
        ...
        try:
            ...
            for speaker in unique_speakers:
                speaker_segments = [s for s in transcription_segments if s.get("speaker") == speaker and s.get("original_audio_file")]
                ...
                if self.model_type == "best":
                    audio_bytes = optimizer.optimize_for_elevenlabs(speaker_segments, speaker, process_temp_dir)
                    service = get_elevenlabs_service()
                else:
                    audio_bytes = optimizer.optimize_for_fish(speaker_segments, speaker, process_temp_dir)
                    service = get_fish_audio_api_service()
    ...
    def _detect_and_create_voice_clones(self, transcription_result: dict, process_temp_dir: str, job_id: str) -> bool:
        ...
        try:
            ...
            if not transcription_segments:
                logger.warning("No transcription segments available")
                return False
            
            unique_speakers = sorted(set(s.get("speaker") for s in transcription_segments if s.get("speaker")))
            if not unique_speakers:
                logger.warning("No speakers detected in segments")
                return False
            
            logger.info(f"Creating voice clones for {len(unique_speakers)} speakers from transcription segments")
            
            self.reference_ids = self._create_speaker_voice_clones(
                transcription_segments, unique_speakers, process_temp_dir, job_id
            )
        ...
        try:
            ...
            if not ai_voice_id and voice_type == 'ai_voice':
                ...
                if user_reference_ids and len(user_reference_ids) > 0:
                    ...
                    logger.info(f"🎙️ Using user-provided reference_id: {ai_voice_id}")
                else:
                    logger.warning(f"⚠️ ai_voice mode but no reference_ids provided for {segment_id}")
    ...
    def _generate_final_output(self, job_id: str, dubbed_segments: list, process_temp_dir: str,
        ...
        try:
            manifest = build_manifest(
                ...
                getattr(self, 'add_subtitle_to_video', False)
            )
            save_manifest_to_dir(manifest, process_temp_dir, job_id)
        except Exception as e:
